...
class Scene extends TurnTableScene
{
    ...
    constructor()
    {
        ...
        // créer les objets
        this.m_Cylinder = new Cylinder(48, this.m_Texture);
        this.m_Rock = new Rock(this.m_Texture, this.m_Cylinder);

        // paramètres de la vue
        this.m_CameraAngleAzimut = 25.0;
        this.m_CameraAngleElevation = 15.0;
        this.m_CameraDistance = 10.0;
        this.m_DisplayCylinder = true;
    ...
    onSurfaceChanged(width, height)
    {
        ...
        this.m_Cylinder.setWindowDimensions(width, height);
    }
    ...
    onKeyDown(code)
    {
        switch (code) {
        case 'T'.charCodeAt(0):
            this.m_DisplayCylinder = !this.m_DisplayCylinder;
            break;
        default:
            // appeler la méthode de la superclasse
    ...
    onDraw(mat4Projection, mat4ModelView)
    {
        ...
        if (this.m_DisplayCylinder) {
            ...
            for (let i=0; i<NbCycles; i++) {
                ...
                this.m_Cylinder.setDepthMaps(this.m_FBOresult.getDepthBuffer(), this.m_FBOpreceeding.getDepthBuffer());

                // dessiner la sphère
                this.m_FBOcurrent.enable();
                gl.clearColor(0.0, 0.0, 0.0, 0.0);
                gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
                this.m_Cylinder.onDraw(mat4Projection, mat4ModelView);
                this.m_FBOcurrent.disable();
