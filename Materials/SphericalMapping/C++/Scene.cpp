...
Scene::Scene() : TurnTableScene(false)
{
    ...
    // créer les objets
    m_Sphere = new Sphere(48,24, m_Texture);
    m_Rock = new Rock(m_Texture);
...
void Scene::onSurfaceChanged(int width, int height)
{
    ...
    m_Sphere->setWindowDimensions(width, height);
}
...
void Scene::onDraw(mat4& mat4Projection, mat4& mat4ModelView)
{
    ...
    if (m_DisplaySphere) {
        ...
        for (int i=0; i<NbCycles; i++) {
            ...
            m_Sphere->setDepthMaps(m_FBOresult->getDepthBuffer(), m_FBOpreceeding->getDepthBuffer());

            // dessiner la sphère
            m_FBOcurrent->enable();
            glClearColor(0.0, 0.0, 0.0, 0.0);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            m_Sphere->onDraw(mat4Projection, mat4ModelView);
            m_FBOcurrent->disable();
