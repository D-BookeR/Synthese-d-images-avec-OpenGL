...
public class CubeCut
{
    private Mesh m_Mesh;

    public CubeCut() throws Exception
    {
        // créer le maillage
        ...
        MeshModuleProcessing transformation = new MeshModuleProcessing(this.m_Mesh);
        MeshModulePhysics physique = new MeshModulePhysics(this.m_Mesh);
        ...
        transformation.bevelVertex(s1, 0.3f, vec3.fromValues(-1,-1,+1));
//        transformation.bevelVertex(s2, 0.3f, vec3.fromValues(+1,-1,+1));
//        transformation.bevelVertex(s3, 0.3f, vec3.fromValues(+1,-1,-1));
//        transformation.bevelVertex(s4, 0.3f, vec3.fromValues(-1,-1,-1));
//        transformation.bevelVertex(s5, 0.3f, vec3.fromValues(-1,+1,+1));
        transformation.bevelVertex(s6, 0.3f, vec3.fromValues(+1,+1,+1));
        transformation.bevelVertex(s7, 0.3f, vec3.fromValues(+1,+1,-1));
//        transformation.bevelVertex(s8, 0.3f, vec3.fromValues(-1,+1,-1));

        // simplifier le maillage pour enlever les triangles inutiles
        m_Mesh.computeNormals();
        reduction.reduxCost(1e-6f);
