...
int main(int argc,char **argv)
{
    ...
    GLFWwindow* window = glfwCreateWindow(960,480, "Livre OpenGL", NULL, NULL);
    if (window == nullptr) {
    ...
    glfwSetWindowTitle(window, "Cameras - Stereo");
    ...
    std::cout << "Left button to turn around" << std::endl;
    std::cout << "Mid button or Z and S keys to zoom" << std::endl;
    std::cout << "Right button or Q,D,A,W keys to shift" << std::endl;

    // boucle principale
    try {
        onSurfaceChanged(window, 960,480);
        do {
            // dessiner
