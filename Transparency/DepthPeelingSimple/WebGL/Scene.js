...
class Scene extends TurnTableScene
{
    ...
    constructor()
    {
        ...
        // créer le matériau de dessin de la pomme (rouge forcé sinon trop sombre)
        let Kd = vec4.fromValues(2.0, 0.4, 0.2, 0.25);
        this.m_TransparentMaterial = new TransparentMaterial(Kd);
    ...
    onSurfaceChanged(width, height)
    {
        ...
        this.m_GBuffer = new FrameBufferObject(width*K, height*K, gl.TEXTURE_2D, gl.TEXTURE_2D);

        // FBO pour la première passe du dessin des objets transparents (alternances entre les deux)
        this.m_PreviousGBuffer = new FrameBufferObject(width*K, height*K, gl.TEXTURE_2D, gl.TEXTURE_2D);
        this.m_CurrentGBuffer = new FrameBufferObject(width*K, height*K, gl.TEXTURE_2D, gl.TEXTURE_2D);
    ...
    onDrawFrame()
    {
        ...
        // NumLayers est définie par un input dans le document HTML, voir onNumLayersChanged() dans main.html
        for (let i=0; i<NumLayers; i++) {
            ...
            this.m_FBOlayers.enable();
            gl.enable(gl.BLEND);
            this.m_CurrentGBuffer.onDraw(gl.COLOR_ATTACHMENT0);
            gl.disable(gl.BLEND);
            this.m_FBOlayers.disable();
