...
Texture360::Texture360(std::string base, int number, GLenum filtering)
{
    ...
    if (number == 1) {
        m_Textures.push_back(new Texture2D(base, filtering));
    } else {
        for (int i=0; i<number; i++) {
            ...
            m_Textures.push_back(new Texture2D(image_filename.str(), filtering));
        }
    }
}
...
Texture2D* Texture360::select(float angle)
{
    if (m_TexturesNumber <= 1) {
        ...
        int number = floor(angle * m_TexturesNumber);
...
/**
 * supprime les textures allouées
 */
