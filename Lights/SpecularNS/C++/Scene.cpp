...
Scene::Scene() : TurnTableScene(false)
{
    ...
    m_BlinnMaterial = new PhongMaterial(lights_count, vec3::fromValues(0.1,0.6,1.0), vec3::fromValues(1.0,1.0,1.0), 512);
    m_RedMaterial = new PhongMaterial(lights_count, vec3::fromValues(0.7,0.2,0.1), vec3::fromValues(0.3,0.3,0.3), 16);
    m_YellowMaterial = new PhongMaterial(lights_count, vec3::fromValues(0.6,0.7,0.0), vec3::fromValues(0.7,0.7,0.7), 96);

    // créer les objets
    m_AppleB = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_BlinnMaterial, 0.01);
    m_RedApple = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_RedMaterial, 0.01);
    m_YellowApple = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_YellowMaterial, 0.01);
...
void Scene::onDraw(mat4& mat4Projection, mat4& mat4ModelView)
{
    ...
    for (int i=0; i<lights_count; i++) {
        ...
        m_RedMaterial->setLightPositionColor(i, position, m_LightsColors[i]);
        m_YellowMaterial->setLightPositionColor(i, position, m_LightsColors[i]);
    }
