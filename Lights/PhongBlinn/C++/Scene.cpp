#include <GL/glew.h>
#include <GL/gl.h>

#include <iostream>

#include <utils.h>
#include <MeshObjectFromObj.h>

#include "Scene.h"



/**
 * Constructeur
 */
Scene::Scene() : TurnTableScene(false)
{
    // coordonnées des sources de lumières dans le repère global
    m_LightsPositions.push_back(vec4::fromValues( 5.0, 0.5,-9.0, 1.0));
    m_LightsPositions.push_back(vec4::fromValues(-6.0, 6.0, 4.0, 1.0));
    int lights_count = m_LightsPositions.size();

    // couleurs des lampes
    m_LightsColors.push_back(vec3::fromValues(1.0, 1.0, 1.0));
    m_LightsColors.push_back(vec3::fromValues(1.0, 1.0, 1.0));

    // créer les matériaux pour les pommes
    m_PhongMaterial = new PhongMaterial(lights_count, vec3::fromValues(0.1,0.6,1.0), vec3::fromValues(0.5,0.5,0.5), 64);
    m_BlinnMaterial = new BlinnMaterial(lights_count, vec3::fromValues(0.7,0.2,0.1), vec3::fromValues(0.5,0.5,0.5), 64);

    // créer les objets
    m_PhongApple   = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_PhongMaterial, 0.01);
    m_AppleBlinn   = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_BlinnMaterial, 0.01);
    m_PhongGround = new MeshObjectFromObj("data/models/TerrainSimple", "Terrain.obj", m_PhongMaterial, 10.0);
    m_BlinnGround = new MeshObjectFromObj("data/models/TerrainSimple", "Terrain.obj", m_BlinnMaterial, 10.0);
    m_Star = new Star(0.5);

    // configurer les modes de dessin
    glEnable(GL_DEPTH_TEST);
    glEnable(GL_CULL_FACE);

    // couleur du fond
    glClearColor(0.4, 0.4, 0.4, 0.0);

    // paramètres de la vue
    m_CameraAngleAzimut = 30.0;
    m_CameraAngleElevation = 10.0;
    vec3::set(m_CameraPivot, 0.0, -0.4, 0.0);
    m_CameraDistance = 18.0;
}


/**
 * appelée quand la taille de la vue OpenGL change
 * @param width : largeur en nombre de pixels de la fenêtre
 * @param height : hauteur en nombre de pixels de la fenêtre
 */
void Scene::onSurfaceChanged(int width, int height)
{
    // appeler la méthode de la superclasse
    TurnTableScene::onSurfaceChanged(width, height);

    // matrice de projection (champ de vision)
    mat4::perspective(m_Mat4Projection, Utils::radians(10.0), (float)width / height, 0.1, 50.0);
}


/**
 * dessine les objets de la scène
 * @param mat4Projection : matrice de projection
 * @param mat4ModelView : matrice de vue
 */
void Scene::onDraw(mat4& mat4Projection, mat4& mat4ModelView)
{
    // matrice temporaire pour positionner chaque objet
    mat4 mat4ModelViewTmp = mat4::create();

    // transformer la position des lampes et la fournir aux matériaux
    vec4 position = vec4::create();
    int lights_count = m_LightsPositions.size();
    for (int i=0; i<lights_count; i++) {
        // calculer la position de la lampe
        vec4::transformMat4(position, m_LightsPositions[i], mat4ModelView);
        m_PhongMaterial->setLightPositionColor(i, position, m_LightsColors[i]);
        m_BlinnMaterial->setLightPositionColor(i, position, m_LightsColors[i]);

        // dessiner une étoile là où est la lampe
        mat4::translate(mat4ModelViewTmp, mat4ModelView, vec3::fromVec(m_LightsPositions[i]));
        m_Star->onDraw(mat4Projection, mat4ModelViewTmp);
    }

    // côté gauche Blinn
    mat4::translate(mat4ModelViewTmp, mat4ModelView, vec3::fromValues(-10.0, 0.0, 0.0));
    m_BlinnGround->onDraw(mat4Projection, mat4ModelViewTmp);
    mat4::translate(mat4ModelViewTmp, mat4ModelView, vec3::fromValues(-1.0, 0.0, 0.0));
    mat4::rotateY(mat4ModelViewTmp, mat4ModelViewTmp, Utils::radians(Utils::Time * 15.0));
    m_AppleBlinn->onDraw(mat4Projection, mat4ModelViewTmp);

    // côté droit Phong
    mat4::translate(mat4ModelViewTmp, mat4ModelView, vec3::fromValues( 10.0, 0.0, 0.0));
    m_PhongGround->onDraw(mat4Projection, mat4ModelViewTmp);
    mat4::translate(mat4ModelViewTmp, mat4ModelView, vec3::fromValues( 1.0, 0.0, 0.0));
    mat4::rotateY(mat4ModelViewTmp, mat4ModelViewTmp, Utils::radians(Utils::Time * 15.0));
    m_PhongApple->onDraw(mat4Projection, mat4ModelViewTmp);
}


/** supprime tous les objets alloués */
Scene::~Scene()
{
    delete m_AppleBlinn;
    delete m_PhongApple;
    delete m_PhongGround;
    delete m_BlinnGround;
    delete m_Star;
    delete m_PhongMaterial;
    delete m_BlinnMaterial;
}
