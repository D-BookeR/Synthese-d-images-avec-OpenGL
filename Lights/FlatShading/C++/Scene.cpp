...
Scene::Scene() : TurnTableScene(false)
{
    ...
    m_PhongMaterial = new PhongMaterial(lights_count, vec3::fromValues(0.1,0.6,1.0), vec3::fromValues(0.8,0.8,0.8), 32);
    m_GouraudMaterial = new GouraudMaterial(lights_count, vec3::fromValues(0.7,0.1,0.2), vec3::fromValues(0.8,0.8,0.8), 32);
    m_FlatMaterial = new FlatMaterial(lights_count, vec3::fromValues(0.1,0.7,0.2), vec3::fromValues(0.8,0.8,0.8), 32);

    // créer les objets
    m_PhongApple   = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_PhongMaterial, 0.01);
    m_AppleGouraud = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_GouraudMaterial, 0.01);
    m_AppleFlat    = new MeshObjectFromObj("data/models/Apple", "apple.obj", m_FlatMaterial, 0.01);
...
void Scene::onDraw(mat4& mat4Projection, mat4& mat4ModelView)
{
    ...
    for (int i=0; i<lights_count; i++) {
        ...
        m_GouraudMaterial->setLightPositionColor(i, position, m_LightsColors[i]);
        m_FlatMaterial->setLightPositionColor(i, position, m_LightsColors[i]);
    }
...
{
    delete m_AppleFlat;
    delete m_AppleGouraud;
    delete m_PhongApple;
    delete m_FlatMaterial;
    delete m_GouraudMaterial;
