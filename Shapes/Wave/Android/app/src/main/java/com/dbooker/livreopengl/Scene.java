...
public class Scene extends TrackBallScene
{
    ...
    public Scene() throws Exception
    {
        ...
        for (int segment=segments_count; segment>=0; segment--) {
            ...
            float r = 40.0f * segment/segments_count;
            // calculer y = f(r)
            float y = -5.0f;
            if (segment > 0) {
                y = (float) (-5.0 * Math.sin(r) / (r*r));
            }
            r = r / 10.0f;
            border.add(vec2.fromValues(r,y));
        }
        ...
        // modes de dessin
        m_MeshStructureDraw = new MeshStructureDraw(m_Object.getMesh(), m_Material, null, null, null, 0.1f);
    }
