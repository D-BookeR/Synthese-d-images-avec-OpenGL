...
import com.dbooker.livreopengl.misc.TrackBallScene;
...
public class Scene extends TrackBallScene
{
    private static final String TAG = "LivreOpenGL";

    private Lathe m_Object;
    ...
    public Scene() throws Exception
    {
        ...
        // créer le contour à faire tourner
        ArrayList<vec2> border = new ArrayList<vec2>();
        border.add(vec2.fromValues(0.0, -1.0));
        border.add(vec2.fromValues(0.4, -1.0));
        border.add(vec2.fromValues(0.5, -0.9));
        border.add(vec2.fromValues(0.6,  0.0));
        border.add(vec2.fromValues(0.3,  0.5));
        border.add(vec2.fromValues(0.3,  0.9));
        border.add(vec2.fromValues(0.2,  1.0));
        border.add(vec2.fromValues(0.0,  1.0));

        // créer l'objet
        m_Object = new Lathe(border, 40);
        ...
        quat.setAxisAngle(m_QuatP1P2, vec3.fromValues(1,0,0), Utils.radians(30.0f));
