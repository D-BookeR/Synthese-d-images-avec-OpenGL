...
{
    private PathExtrusion m_Object;
    ...
    Scene() throws Exception
    {
        ...
        ArrayList<vec2> borderXY = new ArrayList<>();
        borderXY.add(vec2.fromValues(-1.0,  0.0));
        borderXY.add(vec2.fromValues(-0.8,  0.1));
        borderXY.add(vec2.fromValues(-0.5,  0.0));
        borderXY.add(vec2.fromValues(-0.4,  0.2));
        borderXY.add(vec2.fromValues(-0.3,  0.2));
        borderXY.add(vec2.fromValues( 0.0,  0.1));
        borderXY.add(vec2.fromValues( 0.2,  0.1));
        borderXY.add(vec2.fromValues(+0.5,  0.2));
        borderXY.add(vec2.fromValues(+0.8,  0.1));
        borderXY.add(vec2.fromValues(+1.0,  0.0));

        // créer la trajectoire de balayage
        ArrayList<vec2> borderZY = new ArrayList<>();
        borderZY.add(vec2.fromValues(-1.5,  0.0));
        borderZY.add(vec2.fromValues(-1.3,  0.0));
        borderZY.add(vec2.fromValues(-1.0, -0.1));
        borderZY.add(vec2.fromValues(-0.5, -0.2));
        borderZY.add(vec2.fromValues( 0.0, -0.1));
        borderZY.add(vec2.fromValues(+0.5, -0.2));
        borderZY.add(vec2.fromValues(+1.0,  0.0));
        borderZY.add(vec2.fromValues(+1.2,  0.1));
        borderZY.add(vec2.fromValues(+1.5,  0.0));
        ...
        m_CameraAngleAzimut = 135.0f;
        m_CameraAngleElevation = 30.0f;
        m_CameraDistance = 10.0f;

        // modes de dessin
        m_MeshStructureDraw = new MeshStructureDraw(m_Object.getMesh(), m_Material, null, null, null, 0.2f);
    }
    ...
    public void onKeyDown(char code)
    {
        ...
        case 'Y':
            ...
            break;
        case 'U':
            // NB: les normales des faces ne sont pas calculées, donc elles n'apparaitront pas, void PathExtrusion.java
