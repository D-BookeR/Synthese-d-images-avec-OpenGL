...
Scene::Scene() : TurnTableScene(true)
{
    ...
    m_CameraAngleAzimut = -23.0;
    m_CameraAngleElevation = 13.0;
    vec3::set(m_CameraPivot, 0.1, 0.0, -0.2);
    m_CameraDistance = 30;
...
void Scene::onSurfaceChanged(int width, int height)
{
    ...
    m_FBOimage = new FrameBufferObject(width * scale, height * scale, GL_TEXTURE_2D, GL_RENDERBUFFER);

    // fournir le ratio largeur/hauteur au traitement
    m_Lens->setRatio((float)width / height);
}
...
void Scene::onDrawFrame()
{
    ...
    //m_FBOimage->onDraw(GL_DEPTH_ATTACHMENT);

    // à ce stade, on a une image nette dans this.m_FBOimage, on va la distordre
    m_Lens->process(m_FBOimage, 0.5+0.5*cos(Utils::Time));
}
